...
def train_one_epoch(model, teacher, mse_criterion, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    model.train()
    teacher.eval()
    ...
    metric_logger.add_meter("logits_loss", new_utils.SmoothedValue(window_size=10, fmt="{value}"))
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            logits_loss = mse_criterion(output, tea_logits.detach())
            cls_loss = criterion(output, target)
            loss = cls_loss + logits_loss
        ...
        metric_logger.meters["logits_loss"].update(logits_loss.item(), n=batch_size)
